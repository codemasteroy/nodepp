...
EPP.prototype.processRegistryLock = function(data, namespace) {
    var registryLockData = {};
    if (data.hasOwnProperty('type')) registryLockData._attr = { "type": data.type };
    if (data.hasOwnProperty('numbertosend')) registryLockData[namespace + ':numbertosend'] = data.numbertosend;
    if (data.hasOwnProperty('authkey')) registryLockData[namespace + ':authkey'] = data.authkey;

    if (data.hasOwnProperty('smsnumber')) registryLockData[namespace + ':smsnumber'] = data.smsnumber;
...
EPP.prototype.processAuthInfo = function(data, namespace) {
    ...
    if (authType === "string" || authType === "number") {
        ...
        if (!data.hasOwnProperty('pw') && !data.hasOwnProperty('pwregistranttransfer')) throw new Error('pw or pwregistranttransfer is required!');
        if (data.hasOwnProperty('pw')) {
            authInfo = data.pw;
            authInfoData[pwKey] = {};
        }
        if (data.hasOwnProperty('pwregistranttransfer')) {
            pwKey = [namespace, 'pwregistranttransfer'].join(':');
            authInfo = data.pwregistranttransfer;
            authInfoData[pwKey] = {};
        }
...
EPP.prototype.processPostalAddressItem = function(curAddr) {
    ...
    if (curAddr.hasOwnProperty('sp')) preppedCurAddr["contact:sp"] = curAddr.sp;
...
EPP.prototype.processPostalInfoItem = function(postalInfo) {
    ...
    if (postalInfo.hasOwnProperty('firstname')) processedPostal["contact:firstname"] = postalInfo.firstname;
    if (postalInfo.hasOwnProperty('lastname')) processedPostal["contact:lastname"] = postalInfo.lastname;
    if (postalInfo.hasOwnProperty('isfinnish')) processedPostal["contact:isfinnish"] = postalInfo.isfinnish;
    if (postalInfo.hasOwnProperty('org')) processedPostal["contact:org"] = postalInfo.org;
    if (postalInfo.hasOwnProperty('birthDate')) processedPostal["contact:birthDate"] = postalInfo.birthDate;
    if (postalInfo.hasOwnProperty('identity')) processedPostal["contact:identity"] = postalInfo.identity;
    if (postalInfo.hasOwnProperty('registernumber')) processedPostal["contact:registernumber"] = postalInfo.registernumber;
...
EPP.prototype.processContactData = function(data) {
    ...
    var keys = ["voice", "email", "role", "type", "legalemail"];
...
EPP.prototype.updateDomain = function(data, transactionId) {
    ...
    if (change) {
        ...
        if (change.hasOwnProperty('registrylock')) changeData['domain:registrylock'] = this.processRegistryLock(change.registrylock, 'domain');
...
EPP.prototype.checkBalance = function(data, transactionId) {
    var config = this.config;
    var xml = this.eppCommand({
        "check": {
            "balance": ""
